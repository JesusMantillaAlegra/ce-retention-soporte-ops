...
export async function fetchCsat({ token, periodStart, periodEnd }) {
  ...
  // OJO (3-sep-2026, aislado con diag_csat_bisect.mjs): esta propiedad es de
  // tipo 'date' en HubSpot (no 'datetime'), y la Search API exige que los
  // filtros GTE/LTE sobre una propiedad 'date' vengan como timestamp en
  // milisegundos a medianoche UTC -- un string ISO con hora (lo que se
  // manda para las demás fechas del tablero, que sí son 'datetime') le
  // devuelve 400 "There was a problem with the request" sin más detalle.
  // Por eso todos los cubos quedaban con csat en null desde que se armó
  // esta función: el error se atrapaba en construirCubo() y nunca se veía.
  const aMedianocheUTC = (iso) => {
    const d = new Date(iso);
    d.setUTCHours(0, 0, 0, 0);
    return String(d.getTime());
  };
  const construirGrupos = (inicio, fin) => [{
    filters: [
      ...filtrosBase,
      PIPELINE_FILTER,
      { propertyName: csatProp, operator: 'HAS_PROPERTY' },
      { propertyName: 'fecha_de_la_ultima_encuestra_ces_csat', operator: 'GTE', value: aMedianocheUTC(inicio) },
      { propertyName: 'fecha_de_la_ultima_encuestra_ces_csat', operator: 'LTE', value: aMedianocheUTC(fin) },
    ],
  }];

  // Trocea por mes igual que las demás fetch* -- dentro de los cubos (un
  // mes calendario a la vez) esto ya era un solo trozo y no cambia nada,
  // pero permite pedir CSAT sobre un rango de varios meses (ej. el chequeo
  // de /api/diagnostico contra el año completo) sin chocar con el límite
  // de 10.000 resultados de la Search API (ver OJO 3-sep-2026 en
  // searchAllPorMeses).
  const tickets = await searchAllPorMeses(token, construirGrupos, [csatProp], periodStart, periodEnd);

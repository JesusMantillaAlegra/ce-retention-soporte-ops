...
// ya cerraron.
// Actualizado 31-ago-2026: se reemplaza por la suma exacta del pull diario
// usado para los bloques de 5 dias (CREADOS_5D_HARDCODE) -- el Corte 2
// (27-ago-2026) quedaba desactualizado para los meses mas recientes (jun,
// jul, ago), lo que hacia que el acumulado de "Creados vs Cerrados" al
// filtrar un solo mes no coincidiera con el KPI de Volumen de ese mismo mes.
// Con esta fuente unica, ambos coinciden por construccion.
const VOLUMENES_HARDCODE = [5953, 5280, 6468, 6230, 6020, 5324, 5452, 4809];
...
const FCR_CERRADOS_REABIERTOS_HARDCODE = [171, 150, 166, 231, 233, 214, 213, 136];
...
function hardcodeMetrics(start, end) {
  ...
  const hs = { volumen, reopen, cerrados: fcrCerrados, fcr, fcrCerradosReabiertos };
  ...
  if (idx.length === 1) {
    const i = idx[0];
    // Acumulado dentro del mes (no el total de cada bloque de 5 días
    // suelto) — así el último punto llega al total real del mes (el mismo
    // que muestra el gráfico cuando se filtran varios meses) y el gráfico
    // cumple su propósito de detectar backlog: si la línea de Cerrados
    // queda por debajo de la de Creados, hay tickets abiertos acumulándose.
    // Pedido de Noa, 31-ago-2026 (el corte por bloque solo, sin acumular,
    // no llegaba ni cerca del total mensual y confundía).
    const acumular = (arr) => {
      let acc = 0;
    ...
    creadosVsCerrados = {
      ...
      creados: acumular(CREADOS_5D_HARDCODE[i]),
      cerrados: acumular(CERRADOS_5D_HARDCODE[i]),
    };
    primeraRespuesta = {
      meses: BUCKETS_5D_LABELS_HARDCODE[i],

...
// para 2026-01-01 a 2026-08-27. Ver HARDCODE_TEMPORAL arriba.
const SEMANAS_HARDCODE = [
...
const VOLUMENES_HARDCODE = [
  311, 1298, 1416, 1507, 1459,
  1365, 1299, 1280, 1348,
  1519, 1498, 1360, 1428, 1030,
  1606, 1428, 1485, 1556,
  1519, 1562, 1269, 1411,
  1494, 1222, 1103, 1162, 1158,
  1304, 1141, 1122, 1327,
  1162, 1191, 1162, 917,
];
// No se tiene el tiempo de cierre semana por semana desde MCP — se usa el
// promedio general (8,98 días ≈ 215,5 h) como aproximación temporal en
// todas las semanas, solo para que el gráfico de tendencia no quede vacío.
const CIERRE_HORAS_HARDCODE = SEMANAS_HARDCODE.map(() => 215.5);
...
function hardcodeMetrics(start, end) {
  ...
  ].map((v) => ({ ...v, pct_total: pct(v.tickets, hs.volumen), cierre_horas: 215.5 }));
  const tendencia = { weeks: SEMANAS_HARDCODE, volumes: VOLUMENES_HARDCODE, close_hours: CIERRE_HORAS_HARDCODE };

...
export async function buildMetrics({ periodStart, periodEnd } = {}) {
  ...
  return {
    ...
    kpis: {
      ...
      // FCR "(no re-open)" (pedido de Lauren, 31-ago-2026): en vez de su
      // propio numerador/denominador (cerrados − cerrados-reabiertos sobre
      // cerrados), la fórmula pasa a ser 1 − %reopen — el mismo % de reopen
      // que ya se muestra arriba, solo invertido. Se mantiene el % de
      // reopen sobre volumen (no sobre cerrados) porque es el que ya está
      // validado con Estefanía.
      fcr: {
        valor_pct: Number((100 - pct(hs.reopen, hs.volumen, 2)).toFixed(2)),
        gestionados_primer_contacto: hs.fcr,
        cerrados: hs.cerrados,
        fuente: 'hubspot',
        nota: 'FCR (no re-open): 1 − % de reopen',
      },
      csat: {

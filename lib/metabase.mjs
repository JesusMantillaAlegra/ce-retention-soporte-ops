...
async function mbGet(path) {
  ...
  if (!key) throw new Error('Falta METABASE_API_KEY');
  const res = await fetch(`${baseUrl()}${path}`, {
    headers: { 'x-api-key': key, 'Content-Type': 'application/json' },
  });
  if (!res.ok) {
    throw new Error(`Metabase GET ${path} falló (${res.status}): ${(await res.text()).slice(0, 300)}`);
  }
  return res.json();
}
...
async function mbPost(path, body) {
  ...
  if (!res.ok) {
    throw new Error(`Metabase POST ${path} falló (${res.status}): ${(await res.text()).slice(0, 300)}`);
  }
  return res.json();
}
